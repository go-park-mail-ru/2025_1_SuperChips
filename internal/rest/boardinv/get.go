...
//	@Summary		Get invitation links to the board
//	@Description	Get invitation links to the board with ID with parameters for authenticated user who is author
//	@Tags			boards invitations
//	@Produce		json
//	@Security		jwt_auth
//	@Param			board_id	path		int										true	"ID of the board"
//	@Success		200			{object}	ServerResponse{data=domain.LinkParams}	"Array of links with parameters"
...
//	@Router			/api/v1/boards/{id}/invitations [get]
func (b *BoardInvHandler) GetInvitationLinks(w http.ResponseWriter, r *http.Request) {
	...
	links, err := b.BoardInvService.GetInvitationLinks(ctx, boardID, userID)
	if err != nil {
		handleBoardInvError(w, err)
		return
	}
